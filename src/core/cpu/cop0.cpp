...
void checkInterrupt() {
    if (status.cie && (status.im & cause.ip)) cpu::doInterrupt();
}
...
u32 get(u32 idx) {
    ...
    switch (idx) {
        ...
        case static_cast<u32>(COP0Reg::PRId): return 1; // Value for CXD8530BQ CPU
        default:
...
void set(u32 idx, u32 data) {
    switch (idx) {
        ...
        case static_cast<u32>(COP0Reg::Status  ):
            ...
            break;
        case static_cast<u32>(COP0Reg::Cause):
            cause.ip = (cause.ip & 0xFC) | ((data >> 8) & 3); // IP[1:0] are R/W
